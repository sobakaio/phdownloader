...
export function parseM3U8(text, baseUrl) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line || (line.startsWith('#EXT') && !line.startsWith('#EXTINF'))) {
      if (line.startsWith('#EXT-X-STREAM-INF:')) {
        ...
        result.targetDuration = parseInt(line.slice('#EXT-X-TARGETDURATION:'.length), 10) || null;
    ...
    // Reject non-URI lines (HTML error pages, stray text): a valid M3U8 URI
    // line contains no whitespace or markup.
...
/**
 * Pick the variant matching a previously-listed format. `preferred` is either
 * a variant URL (string) or a format descriptor from the quality dropdown
 * ({bandwidth, height}): match by exact bandwidth, then exact height, then
 * closest height. Falls back to the highest-bandwidth variant.
 */
export function pickVariant(parsed, preferred) {
  if (!parsed || !parsed.variants.length) return null;
  if (typeof preferred === 'string') {
    const hit = parsed.variants.find((v) => v.url === preferred);
    if (hit) return hit;
  } else if (preferred) {
    const bw = preferred.bandwidth != null ? preferred.bandwidth : null;
    const h = preferred.height != null ? preferred.height : null;
    let hit = null;
    if (bw != null) hit = parsed.variants.find((v) => v.bandwidth === bw);
    if (!hit && h != null) hit = parsed.variants.find((v) => v.height === h);
    if (!hit && h != null) {
      hit = [...parsed.variants].sort(
        (a, b) => Math.abs((a.height || 0) - h) - Math.abs((b.height || 0) - h))[0];
    }
    if (hit) return hit;
  }
