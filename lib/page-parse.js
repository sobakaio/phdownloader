...
export function extractJsVars(block) {
  const jsVars = {};
  // Pick out each `var <identifier> = <value>` (value runs to the next
  // semicolon) regardless of surrounding statements, so stray non-assignment
  // code cannot corrupt a variable name.
  for (const m of String(block).matchAll(/\bvar\s+([\w$]+)\s*=\s*([^;]+)/g)) {
    jsVars[m[1]] = parseJsValue(m[2], jsVars);
  }
  return jsVars;
}
...
export function extractPageData(html) {
  ...
  const jm = html.match(/\bvar\s+(?:media|quality|qualityItems)\w*\s*=[^\n]*/);
  if (jm) {
